...
mod2 <- glm(mpg ~ poly(disp,2), data = df)
...
add_predictions(df,mod2) %>%  # add_pred adds a new column with your predictions
...
modrows <- unlist(caret::createDataPartition(df$mpg,p = .5))
...
# plot predicted probability of being a "v engine" as hp increases: ####

preds <- add_predictions(df,mod2,type = "response") # make new dataframe with predictions from model (mod2)

# plot the predictions over hp
ggplot(preds, aes(x=hp,y=pred)) +
  geom_smooth() +
  labs(y="Predicted probability of being a \"V-engine\" (based on mod2)",
       x="Horsepower")
